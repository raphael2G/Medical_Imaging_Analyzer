...
def train_model(model, n_epochs, batch_size, train_ds, validation_ds=None, test_ds=None, with_plot=True, 
    ...
    if validation_ds != None: 
        with_validation=True
        ds_validation_batch = validation_ds.batch(len(validation_ds)-1)
        print('Begining Training with Validation')
    else: 
        with_validation=False
        print('Begining Training without Validation')

    if test_ds != None: 
        with_test=True
        ds_test_batch = test_ds.batch(len(validation_ds)-1)
        print('Begining Training with Testing')
    else: 
        with_test=False
        print('Begining Training without Testing')
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % update_increment == 0:
            print('Epoch: %i' %epoch, 'Accuracy: %.4f' %epoch_accuracy.result(), 'Loss: %.6f' %epoch_loss.result())
